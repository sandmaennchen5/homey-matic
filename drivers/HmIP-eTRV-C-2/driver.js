...
class HomematicDriver extends Driver {
    ...
    onInit() {
        ...
        this.homematicTypes = ['HmIP-eTRV-C-2'];
        this.log(this.homematicTypes.join(','), 'has been inited');

        this.homey.flow.getActionCard('HmIP-eTRV-C-2-thermostat_set_weekprofile')
            .registerRunListener((args, state) => {
                return args.device.triggerCapabilityListener('homematic_thermostat_weekprofile', args.weekprofile, {})

            })

        this.homey.flow.getActionCard('HmIP-eTRV-C-2-thermostat_set_mode')
            .registerRunListener((args, state) => {
